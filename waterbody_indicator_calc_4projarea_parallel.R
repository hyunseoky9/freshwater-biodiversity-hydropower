...
wbd_class_identify <- function(comid)
{
  ...
  # mode function. (which element has the most amount in a list?) 
...
for(i in 5) #looplength)
{
  ...
  wbd_rid_idx = which((newdata$wbd_classes != 'NA' & newdata$wbd_classes != 'LakePond' & newdata$wbd_classes != 'Reservoir') | (is.na(newdata$wbd_classes) & newdata$waterbody==1))
