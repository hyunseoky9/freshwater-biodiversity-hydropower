...
#  narows = apply(spdata,1,function(x) any(is.na(x)))
#  spdata = spdata[-narows,]
#  numoccs[i] = length(which(spdata$wbmID_30sec_netsymdiff < threshold))
#}
#idx0 = which(numoccs>30)
#length(idx0)
...
reps = c(3,39,12,100,44,20,4,1,93,24) # first one is control, rest represents each error types
...
#i=idx0[reps[3]]
#94
c(139, 218,223, 314,419)
for(i in idx0[c(139,218,223,314,419)])
{
  ...
  # using maxent.jar
  #occ = dplyr::select(occ,-c(numday_above_tmax))
  #bg = dplyr::select(bg,-c(numday_above_tmax))
  ...
  s_mtp = rep(NA,length(projpred))
  s_mtp[which(projpred>mtp)] = 1
  s_mtp[which(projpred<=mtp)] = 0
  
  # maxsss Maximum Sensitivity plus Sensibility threshold of the model (Liu and White 2015)
  # tss definition in https://groups.google.com/g/maxent/c/33ESpiEieNk
  if(class(mod.seq)[1]=='MaxEnt')
  {
  ...
  s_maxsss = rep(NA, length(projpred))  
  ...
  {
  projectionareatffilename = sprintf('D:/sdm_modeling/by_sp predictor data temporary storage/%s/%s/%s/%s_projarea.csv','pristine_gcm_reservoir',gcmver,'future',spnamedata$name[i]) # future reservoir proj
  projtf = fread(projectionareatffilename)
  
  projectionareaflowfilename = sprintf('D:/sdm_modeling/by_sp predictor data temporary storage/%s/%s/%s/%s_projarea.csv','pristine w gcm',gcmver,'future',spnamedata$name[i])
  projflow = fread(projectionareaflowfilename)
  proj = cbind(projntf,projtf[,c('numday_above_tmax','numday_below_tmin','dd90_5c','dd90_8c','dd90_10c','dd120_5c','dd120_8c','dd120_10c','dd150_5c','dd150_8c','dd150_10c','avgtemp')],
                  projflow[,c('maxflow','maxflowdate','minflow','minflowdate','avgflow','maxminflowdiff')])
  ...
  {
  projectionareatffilename = sprintf('D:/sdm_modeling/by_sp predictor data temporary storage/%s/%s/%s/%s_projarea.csv','pristine w gcm',gcmver,'current',spnamedata$name[i]) # current pristine proj  
  projtf = fread(projectionareatffilename)
  ...
  {
    projectionareatffilename = sprintf('D:/sdm_modeling/by_sp predictor data temporary storage/%s/%s/%s/%s_projarea.csv','pristine w gcm',gcmver,'future',spnamedata$name[i]) # current pristine proj  
    projtf = fread(projectionareatffilename)
    proj = cbind(projntf,projtf[,c('numday_above_tmax','numday_below_tmin','dd90_5c','dd90_8c',
                                   'dd90_10c','dd120_5c','dd120_8c','dd120_10c','dd150_5c',
                                   'dd150_8c','dd150_10c','avgtemp','maxflow','maxflowdate',
...
runtimefilename = sprintf('G:/My Drive/research/sdm_modeling/sdm_results/%s/runtime.csv',gcmver)
