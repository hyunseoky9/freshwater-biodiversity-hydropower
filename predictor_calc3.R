...
# difference from predictor_calc2: calculates all the numday above tmax and numday below tmin predictors for every tmin and tmax thresholds
# at the huc level, so I don't have to call the raw temp and flow data everytime I need to calculate tempflow-related predictors when making 
# predictors by species (both projection areas and occpts)
...
for(i in irange)
{
  ...
  for(j in 1:(finish-start + 1))
  {
    ...
    for(k in tmaxs)
    {
      if(all0s_fromhere==0)
      {
        matwhich = apply(cbind(as.matrix(tempd[,2:ncol(tempd)]),rep(k-1,nrow(tempd))),1,function(x) which(x<k))
...
for(i in 1:nrow(spdata))
{
  ...
  for(j in 1:length(hucs))
  {
    ...
    {
      numdayabove_idx = which(grepl('numday_above_tmax',names(hucd)))[1]
      tmaxs_in_colnames = as.numeric(sub('numday_above_tmax_','',names(hucd)[which(grepl('numday_above_tmax',names(hucd)))]))
      ...
      spproj$numday_above_tmax[which(spproj$huc2==hucs[j])] = hucd[idx,numdayabove_idx]
    } else
    {
    ...
    {
      numdaybelow_idx = which(grepl('numday_below_tmin',names(hucd)))[1]
      names(hucd)[which(grepl('\\.2\\.000',names(hucd)))] = 'numday_below_tmin_-2.000'
      tmins_in_colnames = as.numeric(sub('numday_below_tmin_','',names(hucd)[which(grepl('numday_below_tmin',names(hucd)))]))
      numdaybelow_idx = numdaybelow_idx + which(abs(tmin - tmins_in_colnames)== min(abs(tmin-tmins_in_colnames))) - 1
      #tmaxidx = which(tmaxs == tmax)
      spproj$numday_below_tmin[which(spproj$huc2==hucs[j])] = hucd[idx,numdaybelow_idx]
    } else
    {
...
for(i in 1:nrow(spdata))#predictor_done[16:length(predictor_done)])
{
  ...
  projareafilename = sprintf('G:/My Drive/research/sdm_modeling/spdata/projection_area/by_sp/pristine w gcm/current/%s_projarea.csv',spdata$name[i])
