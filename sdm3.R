# running SDM
# latest version since 6/19/2023
# built to fit the predictors where the tempflow related predictors and non-tempflow related predictors are in a separate csv file, and the pristine runs are run by gcms.
...
numoccs = c()
{
  ...
  scenario = 'pristine_gcm_reservoir' # 'pristine w gcm'
  for(i in idx0)
  {
    # read in data
    ...
    # NA BFI values to 0
    sp =sp$BFI[-which(is.na(sp$BFI)),]
    proj =proj$BFI[-which(is.na(proj$BFI)),]
    ...
    if(dataversion == 1)
    {
      bckdata = data.frame(dplyr::select(bckdata,BFI,waterbody,numday_above_tmax,
      ...
      spdf = data.frame(dplyr::select(sp,BFI,waterbody,numday_above_tmax,
                                      numday_below_tmin,dd90_5c,dd90_8c,dd90_10c,
                                      dd120_5c,dd120_8c,dd120_10c,dd150_5c,dd150_8c,
                                      dd150_10c,maxflow,maxflowdate,minflow,
                                      minflowdate,avgtemp,streamorder))
    ...
    {
      bckdata = data.frame(dplyr::select(bckdata,BFI,waterbody,streamorder,numday_above_tmax,dd90_8c,minflow,minflowdate,maxflowdate,avgflow))
      spdf = data.frame(dplyr::select(sp,BFI,waterbody,streamorder,numday_above_tmax,dd90_8c,minflow,minflowdate,maxflowdate,avgflow))
      path2 = sprintf('%s/%s_pristinerun_dfdd90_8c',path2,spnamedata$name[i])
    } else if(dataversion == 3)
    {
      bckdata = data.frame(dplyr::select(bckdata,BFI,waterbody,streamorder,numday_above_tmax,dd90_10c,minflow,minflowdate,maxflowdate,avgflow))
      spdf = data.frame(dplyr::select(sp,BFI,waterbody,streamorder,numday_above_tmax,dd90_10c,minflow,minflowdate,maxflowdate,avgflow))
    ...
    occbg = rbind(spdf,bg)
    occbgp = c(rep(1,nrow(spdf)),rep(0,nrow(bg)))
    ...
    p_addon = proj$p
    # project function info: https://rdrr.io/github/johnbaums/rmaxent/man/project.html
    ...
    s_max3s[which(predalldf$prediction_cloglog>max3strain)] = 1
...
beep(sound=5)
t = proc.time() - ptm
print(sprintf('%.3f hours',t[3]/3600))
}
