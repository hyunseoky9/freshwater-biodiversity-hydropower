...
for(i in irange)
{
  ...
  for(j in 1:(finish-start + 1))
  {
    ...
    flowfilename = sprintf('G:/My Drive/research/sdm_modeling/wbmdata/pristine/Huc%d_CONUS_Output_RiverDischarge_DOERunoff+WBM20WTempPrist_01min_dTS%d_v2.csv',i,yr)
    ...
    if(j==1){
      pred_perhuc = data.frame(wbmID_1min=tempd$ID,
                               ...
                               dd120_10c, dd150_5c, dd150_8c, dd150_10c,
...
# 2. get predictor values for each species projection area using their tmin tmax data. 
...
for(i in c(202,243,263,266))
{
  ...
  tmin = tmins[i]
  tmax = tmaxs[i]
  spproj_og = fread(path1)
  matchidx = which(is.na(spproj_og$copiedfromoldproj))
  if(length(matchidx)>0)
  {
    spproj = spproj_og[matchidx,]
    ...
    spproj$avgtemp = NA
    print(sprintf('huc count: %d',length(hucs)))
    for(j in 1:length(hucs))
    {
      print(sprintf('huc %d of %d',j,length(hucs)))
      ...
      if (calc_predictor2_separately + calc_predictor1_separately > 0)
      {
        ...
        for(k in 1:(finish-start + 1))
        {
          # access the temperature data for those points for the relevant timeframe calculate the temperature and flow related predictor variables:
          yr = (k-1) + start
          tempfilename = sprintf('G:/My Drive/research/sdm_modeling/wbmdata/pristine/Huc%d_CONUS_Output_RiverTemperature_DOERunoff+WBM20WTempPrist_01min_dTS%d_v2.csv',hucs[j],yr)
          tempd =fread(tempfilename,header=TRUE)
          names(tempd)[1] = 'ID'
    ...
    spproj_og$numday_above_tmax[matchidx] = spproj$numday_above_tmax
    spproj_og$numday_below_tmin[matchidx] = spproj$numday_below_tmin
    spproj_og$dd90_5c[matchidx] = spproj$dd90_5c
    spproj_og$dd90_8c[matchidx] = spproj$dd90_8c
    spproj_og$dd90_10c[matchidx] = spproj$dd90_10c
    spproj_og$dd120_5c[matchidx] = spproj$dd120_5c
    spproj_og$dd120_8c[matchidx] = spproj$dd120_8c
    spproj_og$dd120_10c[matchidx] = spproj$dd120_10c
    spproj_og$dd150_5c[matchidx] = spproj$dd150_5c
    spproj_og$dd150_8c[matchidx] = spproj$dd150_8c
    spproj_og$dd150_10c[matchidx] = spproj$dd150_10c
    spproj_og$maxflow[matchidx] = spproj$maxflow
    spproj_og$maxflowdate[matchidx] = spproj$maxflowdate
    spproj_og$minflow[matchidx] = spproj$minflow
    spproj_og$minflowdate[matchidx] = spproj$minflowdate
    spproj_og$avgflow[matchidx] = spproj$avgflow
    spproj_og$maxminflowdiff[matchidx] = spproj$maxminflowdiff
    spproj_og$avgtemp[matchidx] = spproj$avgtemp
    # write out the data in a separate file
    ofilename = sprintf('G:/My Drive/research/sdm_modeling/spdata/projection_area/by_sp/%s_projarea.csv',spdata$name[i])
    write.csv(spproj_og,ofilename,row.names=FALSE)
  }
...
for(i in 120:nrow(spdata))#predictor_done[16:length(predictor_done)])
{
