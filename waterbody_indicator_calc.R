...
for(i in 11:13) #looplength)
{
  ...
  print('wbd class identify no error')
  if(nrow(data)!= nrow(key))
  {
  ...
  wbd_rid_idx = which(data$wbd_classes != 'NA' & data$wbd_classes != 'LakePond' & data$wbd_classes != 'Reservoir'| is.na(data$wbd_classes))
  if(length(wbd_rid_idx))
  {data = data[-wbd_rid_idx,]} # get rid of occpts that are not lakeponds or reservoirs
  
  flowline_rid_idx = which(data$ftype != 'StreamRiver' & data$ftype != 'CanalDitch' & data$ftype != 'ArtificialPath'|is.na(data$ftype))
...
for ( i in wbd_idx[1:ceiling(0.6*length(wbd_idx))])
{
  key$wbd_classes[i] = wbd_class_identify(key$comid[i])
  print(sprintf('%d done',i))
}
key$wbd_classes[wbd_idx] = pbapply(as.matrix(key$comid[wbd_idx]),1,wbd_class_identify)

weirdcomids = comid[which( !comid %in% subset$NHDFlowline_Network$comid)]
dsubset <- subset_nhdplus(comids = c(comid[1],weirdcomids),
                         output_file = subset_file,
                         nhdplus_data = "download", 
                         flowline_only = TRUE,
                         return_data = TRUE, overwrite = TRUE)

ddata = merge(data,key)
