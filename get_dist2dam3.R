...
# * This third version uses HILAREE hydropower subset data instead of Ryan's data.
...
get_origin <- function(first.dam,term.comid,damcomids)
{
  ...
  # damcomids = comids of dams in hilarri's dam data.
  done = 0
...
if(length(which(is.na(hilarri$comid)))>0)
{
  hilarri = hilarri[-which(is.na(hilarri$comid)),] # get rid of na comids  
}
if(length(which(duplicated(hilarri$comid)))>0)
{
  hilarri = hilarri[-which(duplicated(hilarri$comid)),] # get rid of duplicate comids  
}
damreachD = hilarri[,c('comid','u_length_km','d_length_km')]


udd2filename = 'G:/My Drive/research/sdm_modeling/dam data/udd2_wHILARRI.csv'
#udd2info = c()
udd2info = read.csv(udd2filename)
#alreadydone = c()
alreadydone = unique(udd2info$umdam_comid)
error_comids = c(6163241,6711861,3181358,22294810,22049573,15515200,19333763,868040,17078817,944060020,948021110,13297204,19334993,3024846,10025016,15491290) # comids of dams in the hilarri_hy edition that did not work with nhdplustools

unfinished.dam = hilarri$comid[which(!is.na(hilarri$comid))]
unfinished.dam = unfinished.dam[which(!unfinished.dam %in% alreadydone)]
unfinished.dam = unfinished.dam[-which(unfinished.dam %in% error_comids)]
total_dam_num = length(unfinished.dam)


dam.reaches = hilarri$comid
next.dam.id = -1
...
while(length(unfinished.dam))
{
  ...
  if(next.dam.id != -1)
  {
    first.dam = hilarri[which(hilarri$comid == next.dam.id),] # use the one chosen from last round of the loop
  } else {
    select_idx = ceiling(runif(1,0,1)*length(unfinished.dam))
    subj.comid = unfinished.dam[select_idx]
    first.dam = hilarri[which(hilarri$comid == subj.comid),] # pick one at random
  }
  ...
  dm.comids = try({
    get_terminal(first.dam$comid,dam.reaches)
  },silent=TRUE)
  if('try-error' %in% class(dm.comids))
  {
    error_comids = c(error_comids,first.dam$comid)
    print('error in get_terminal')
    next
  }
  ...
  {
    print('udd2val done')
    added_data = data.frame(calc_comid,first.dam$nididfull,first.dam$longitude,first.dam$latitude,first.dam$comid,udd2val)
    names(added_data) = c('comid','NIDID_full','damlong','damlat','umdam_comid','udd2')
...
for(i in 1:length(spname_list))
{
  ...
  if(occ_or_proj)
  {
    ...
    hucnum = i
    projfilename = sprintf("G:/My Drive/research/sdm_modeling/spdata/projection_area/by_huc_nontempflow/huc%d.csv",hucnum)
  ...
  data$udd2hilarri = damdistdata$udd2[matches] # udd=upstream dam distance
  data$udd2hilarri_nidid = damdistdata$NIDID_full[matches]
  data$udd2hilarri_umdamcomid = damdistdata$umdam_comid[matches]
...
# same as above for-loop but for projection areas by species
for(i in 1:length(spname_list))
{
  # **spdata could mean species occurrence points or projection area.
  spname = spnamedata$name[i]
  # for getting predictor values of fitting points.(sp occ pts)
  spfilename = sprintf('G:/My Drive/research/sdm_modeling/spdata/projection_area/by_sp/only_non-tempflow related predictors/%s_projarea_non-tempflowpredictors.csv',spname)
  data = read.csv(spfilename)  
  matches = match(data$comid,damdistdata$comid)
  data$udd2hilarri = damdistdata$udd2[matches] # udd=upstream dam distance
  data$udd2hilarri_nidid = damdistdata$NIDID_full[matches]
