# 2/14/2023 
# same as area restriction 2, but instead of getting the projection area based on the normal, 
# processed occurrence pts, I use occpts b4 processing (except for matching to nhd comids).
...
multi6 = c()
#c(46,120,126,130,150,301,313,316,320,349,385,484,492,508,553,562)
for ( i in 77:85)
{
  spdatafilename = sprintf('G:/My Drive/research/sdm_modeling/spdata/per_sp/no_date_filter/%s_wcomid.csv',spnamedata$name[i])
  ...
  } else
  {
    ...
    for(j in 1:length(relhuc2))
    {
      ...
      if(j==1)
      {
        sp_projarea = huc2D[which(huc2D$comid %in% comid),]
      } else
      {
        sp_projarea = rbind(sp_projarea, huc2D[which(huc2D$comid %in% comid),])
      }
    }
  }
