# make csv file of occurrence data into vector shapefile.
library(rgeos)
library(maptools)
library(proj4)
library(data.table)
...
occurrence.from.shapefile2 <- function(shapefile, occurrence_df, lat, lon, gbifid, species, occurrenceStatus, map_crs = "+proj=longlat +datum=WGS84", mkplot = FALSE){
  ...
  occ.map <- data.frame(occurrence_df[[lon]], occurrence_df[[lat]], occurrence_df[[gbifid]],
                        occurrence_df[[species]], occurrence_df[[occurrenceStatus]])
  print(str(occ.map, 1))
  #simplify column names
  names(occ.map)[1:5] <- c('long', 'lat', 'gbifID','species','occurrenceStatus')
