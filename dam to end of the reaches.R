...
comids = ryan$comid
points = cbind(ryan$lon,ryan$lat)
nonacomids = which(!is.na(ryan$comid))

distdata = as.data.frame(cbind(ryan$comid,NA,NA))
names(distdata) = c('comid','u_length_km','d_length_km')
weird_partl = c() # record weird part lengths
for( i in weird_partl)#nonacomids)
{
  comid = comids[i]
  rawpoint = points[i,]
...
newdata = cbind(ryan,distdata$u_length_km,distdata$d_length_km)
names(newdata)[c((ncol(newdata)-1),ncol(newdata))] = c('u_length_km','d_length_km')

write.csv(newdata,ryanfilename,row.names=FALSE)
head(newdata)
